...
//    cranePump_pub_ = node.advertise<hexapod_msgs::CranePump>( "/cranePump", 10 );
...
void keyboard_to_joy::KeyCallback(const keyboard::Key::ConstPtr& key)
{
   ...
   // button.assign(12,0);
    ...
    if (key->code == 51) // key 3 is_rotation
    {
       // body_command.cmd = body_command.STAND_UP_CMD;
       // body_cmd_pub.publish(body_command);
    ...
    if (key->code ==119) // w, left up
    {
       // body_command.cmd = body_command.STAND_UP_CMD;
       // body_cmd_pub.publish(body_command);
        joy.axes[FORWARD_BACKWARD_AXES] = joy.axes[FORWARD_BACKWARD_AXES] + 0.1;
    ...
    {
        joy.axes[LEFT_RIGHT_AXES] = joy.axes[LEFT_RIGHT_AXES] - 0.1;
        joy_pub.publish(joy);
    }
    ...
    if (key->code ==105) // i, right up
    {
       // body_command.cmd = body_command.STAND_UP_CMD;
       // body_cmd_pub.publish(body_command);
        joy.axes[YAW_ROTATION_AXES] = joy.axes[YAW_ROTATION_AXES] + 0.1;
    ...
    {
        joy.axes[PITCH_ROTATION_AXES] = joy.axes[PITCH_ROTATION_AXES] - 0.1;
        joy_pub.publish(joy);
    }
    ...
    if (key->code == 98) // b, pump
    {
        joy.buttons[9] = !joy.buttons[9];
        joy_pub.publish(joy);
    }
    
    if (key->code == 32) // space, crane
    {
        joy.buttons[10] = !joy.buttons[10];
        joy_pub.publish(joy);
    }
